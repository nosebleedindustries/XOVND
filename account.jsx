...
const { useState: useAccState, useEffect: useAccEffect, useMemo: useAccMemo } = React;
...
function ProfilePanel({ user, profile, setProfile }) {
  const [name, setName] = useAccState(profile.name || user.name || user.email.split("@")[0]);
  const [photo, setPhoto] = useAccState(profile.photo || null);
  const [saved, setSaved] = useAccState(false);
  ...
  return (
    <div className="acc-panel">
      ...
      <div className="sub-actions">
        <a href="subscription.html" className="sub-act primary">Change plan</a>
...
function MessagesPanel({ messages, setMessages, user, showToast }) {
  const [draft, setDraft] = useAccState("");
...
function AccountApp() {
  const { cart, cartOpen, openCart, closeCart, removeAt } = window.useCart();
  const auth = window.useAuth();
  const [loginOpen, setLoginOpen] = useAccState(false);
  const [open, setOpen] = useAccState("profile"); // profile|downloads|subscription|messages
  const [toast, setToast] = useAccState("");
  const showToast = (t) => { setToast(t); setTimeout(() => setToast(""), 2000); };

  const [profile, setProfile] = useAccState(() => readObj(ACC_PROFILE_KEY, {}));
  useAccEffect(() => { writeObj(ACC_PROFILE_KEY, profile); }, [profile]);

  const [dl, setDl] = useAccState(() => readObj(ACC_DL_KEY, null));
  ...
  const [messages, setMessages] = useAccState(() => readObj(ACC_MSG_KEY, SEED_MESSAGES));
  ...
  const rows = [
    ...(isAdmin ? [{ id: "admin", title: "Admin dashboard", sub: "Studio inbox · tickets · purchases · features", ic: Ic.admin, href: "admin.html" }] : []),

...
_mi.rotate(V(0,0,0),V(0,0,1),180); _mi.translate(V(-61,-58,2))   # flip 180deg -> jacks face front; relocated front-left
shapes.append(_mi); print("  loaded MIDI (flipped 180) %d solids"%len(_mi.Solids),flush=True)
shapes.append(load(D+r"\Downloads\display_carrier\display_carrier.step",-137,79.5,22.85,"CARR"))
...
for (x,y) in [(-133.3,73.4),(55.8,73.4),(-133.3,-2.4),(55.8,-2.4)]:
    shapes.append(hexp(x,y,0.0,22.8))
# 6-way IDC ribbon: MIDI board J3 -> carrier J_MIDI (underside)
import FreeCAD
_A=FreeCAD.Vector(-77,-20,8.6); _B=FreeCAD.Vector(-77,0.5,16.0); _d=_B.sub(_A)
